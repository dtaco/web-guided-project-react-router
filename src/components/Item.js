...
export default function Item(props) {
  ...
  //  7 - We need to pull item from items, using a parameter in the URL (:itemID)
  ...
  return (
    <div className='item-wrapper'>
      ...
      <nav className='item-sub-nav'>
        {/*  8 - Here go the NavLinks to `<current url>/shipping` and `<current url>/description` */}
        <NavLink to="description">Description</NavLink>
        <NavLink to="shipping">Shipping</NavLink>
      </nav>

      {/*  9 - Here go the Routes for `<current path>/shipping` and `<current path>/description` */}
